# iREPS Repository Skills

## Purpose

This file contains binding housekeeping rules for the active iREPS repository.

### Root folder definition

In this file, **`<root folder>`** means the root directory of the active repository being worked on in the current chat.

Examples include:
- `C:\dev\ireps-mobile`
- `C:\dev\ireps-web`
- `C:\dev\ireps-pipeline-erfs`

The rules in this file are repository-neutral. Do not hard-code one repository name or path when the same rule is intended to apply to all iREPS repositories.

Before giving or following any instruction involving ZIP creation, ZIP upload, ZIP download, ZIP extraction, file delivery, repository housekeeping, or cleanup, read this file first and apply it.

## Mandatory new-chat repository inspection

At the start of every new iREPS repository chat:
...
6. Inspect the uploaded repository tree and Git state before touching application/source/configuration code or making any repository change.
...
## Mandatory branch-per-workstream policy

1. Every iREPS module, feature, artifact, cleanup, refactor, migration, lint session, documentation task, or other repository workstream must be performed on its own dedicated Git branch.

2. `main` is the integration branch. New work must not be developed directly on `main`.

3. Before starting a workstream:
   - inspect the current branch, commit, and Git status;
   - preserve any legitimate dirty work;
   - never reset, stash, clean, discard, or overwrite legitimate changes merely to create a branch;
   - create or switch to a clearly named work branch before making new workstream changes.

4. Branch names must be descriptive and tied to the workstream, for example:
   - `cleanup/ireps-mobile-cleanup-v1`
   - `fix/meter-discovery-finetune-v1`
   - `chore/lint-cleanup-v1`
   - `feature/targeted-batch-mobile-v1`

5. All implementation changes, generated governance changes, tests, and workstream-specific documentation must be committed on the dedicated work branch.

6. Before merging a work branch:
   - inspect the complete diff;
   - run the agreed tests/verification;
   - confirm no unrelated or accidental files are included;
   - confirm the workstream is complete;
   - obtain explicit user approval to merge when working interactively with ChatGPT.

7. Merge the completed work branch into `main` only after the workstream has passed its agreed verification.

8. After the merge:
   - verify the expected commit is present on `main`;
   - verify the working tree is clean;
   - run any required post-merge smoke check;
   - only then consider the workstream closed.

9. Do not delete the work branch until the merge into `main` has been verified. Branch deletion is optional unless an explicit repository housekeeping rule requires it.

10. If a workstream is already in progress on `main` when this policy is introduced, preserve the current dirty state and create a dedicated branch from that exact state before staging or committing. Do not reset or discard the work.

11. The binding default is:

    `inspect main → create dedicated work branch → implement and test → commit on branch → verify → merge to main → verify clean main`
...
4. Every delivery ZIP must preserve repository-relative paths so that extracting it into `<root folder>` places each file directly into its correct repository folder.

5. A file must never be placed in an arbitrary folder after extraction. Its path inside the ZIP must match its intended path under the active `<root folder>`.
...
3. The extraction script must:
   ...
   - place every file into its correct path under `<root folder>`;
...
3. The creation script must:
   ...
   - create the ZIP inside `<root folder>\zips`;
...
Before preparing any ZIP-related instruction for an iREPS repository:

1. Read `<root folder>\skills.md`.
