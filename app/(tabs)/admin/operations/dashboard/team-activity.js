...
function getTeamTargets(trn = {}) {
  const directTeamTargets = getAssignmentTargets(trn).filter(
    (target) => target.type === "TEAM",
  );

  if (directTeamTargets.length) return directTeamTargets;

  const fallbackTeamId = cleanText(
    trn?.assignment?.team?.id ||
      trn?.team?.id ||
      trn?.assignment?.assignedTeam?.id,
    "",
  );

  const fallbackTeamName = cleanText(
  ...
  if (fallbackTeamId || fallbackTeamName) {
    return [
      {
        type: "TEAM",
        id: fallbackTeamId || fallbackTeamName,
        name: fallbackTeamName || fallbackTeamId,
      },
    ];
  }
...
function getTeamKey(team = {}) {
  ...
  if (name && name !== "NAv") return `TEAM_NAME:${name.toUpperCase()}`;
...
function createEmptyTeamRow(team = {}) {
  return {
    ...
    teamName: cleanText(team?.name || "No Team"),
    serviceProvider: cleanText(team?.serviceProvider, "NAv"),
...
function touchTeamRow(rowsByTeam, team = {}) {
  const key = getTeamKey(team);

  if (!rowsByTeam[key]) {
    rowsByTeam[key] = createEmptyTeamRow(team);
  }

  const row = rowsByTeam[key];

  if (row.teamName === "NAv" && team.name) row.teamName = cleanText(team.name);
  if (row.serviceProvider === "NAv" && team.serviceProvider) {
    row.serviceProvider = cleanText(team.serviceProvider);
  }
...
function addTrnToTeamRow(row, trn = {}) {
  ...
  getUserTargets(trn).forEach((target) => {
    if (target.id) row.assignedUsers.add(target.id);
  });
...
function buildTeamActivityRows(items = []) {
  const rowsByTeam = {};

  items.forEach((trn) => {
    const serviceProvider = getServiceProviderName(trn);
    const teamTargets = getTeamTargets(trn);

    teamTargets.forEach((team) => {
      const row = touchTeamRow(rowsByTeam, {
        ...team,
        serviceProvider,
      });

      addTrnToTeamRow(row, trn);
    });
  });

  return Object.values(rowsByTeam)
    .map((row) => ({
      ...row,
      userCount: row.assignedUsers.size,
...
export default function TeamActivityScreen() {
  ...
  const teamRows = useMemo(() => buildTeamActivityRows(wmsItems), [wmsItems]);
        ...
        <View style={styles.headerCard}>
          ...
          {loading ? (
            ...
            <View style={styles.headerCountBadge}>
              <Text style={styles.headerCountText}>{teamRows.length}</Text>
            </View>
          )}
        </View>
        ...
        {!scopeReady ? (
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyText}>
              Team Activity is ward-scoped. Select an active ward before viewing
              WMS team activity.
            </Text>
          </View>
        ) : teamRows.length === 0 ? (
          <View style={styles.emptyCard}>
            <MaterialCommunityIcons
              ...
              color="#94a3b8"
            />
            <Text style={styles.emptyTitle}>No team activity</Text>
            <Text style={styles.emptyText}>
              No WMS team activity was found for this ward and date filter.
            </Text>
          </View>
        ) : (
          <>
            <Text style={styles.sectionTitle}>Ward Team Activity</Text>

            {teamRows.map((row) => (
              <TeamActivityCard key={row.key} row={row} />
            ))}
          </>
        )}
      </ScrollView>
      ...
      <View style={styles.teamHeader}>
        ...
        <View style={styles.teamMain}>
          ...
          <Text style={styles.teamMeta}>
            SP: {row.serviceProvider} • Users: {row.userCount}
          </Text>
        </View>
